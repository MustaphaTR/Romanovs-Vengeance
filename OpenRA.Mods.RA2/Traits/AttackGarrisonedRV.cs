...
namespace OpenRA.Mods.RA2.Traits
{
	...
	public class AttackGarrisonedRV : AttackFollow, INotifyGarrisonerEntered, INotifyGarrisonerExited, IRender
	{
		...
		{
			paxFacing.Add(garrisoner, garrisoner.Trait<IFacing>());
			paxPos.Add(garrisoner, garrisoner.Trait<IPositionable>());
			paxRender.Add(garrisoner, garrisoner.Trait<RenderSprites>());
			armaments.AddRange(
				garrisoner.TraitsImplementing<Armament>()
				.Where(a => Info.Armaments.Contains(a.Info.Name)));
		}

		void INotifyGarrisonerExited.OnGarrisonerExited(Actor self, Actor garrisoner)
		{
			paxFacing.Remove(garrisoner);
			paxPos.Remove(garrisoner);
			paxRender.Remove(garrisoner);
			armaments.RemoveAll(a => a.Actor == garrisoner);
		}
